...
- **Fix:** Both the HALT CHECK snippet and the JOB DECLARATION snippet were updated to replace the broken 3-tier ladder with a single explicit agent-substituted `session_id = "REPLACE_WITH_YOUR_SESSION_ID"` placeholder and a fail-loud guard (`raise SystemExit(...)`) that prevents execution if the placeholder is left unsubstituted. The agent is directed to copy its session id from the `Session ID:` line in its system prompt.
- **Scope:** Only the two job-marker snippets were changed. The TASK CLASSIFICATION snippet shares the same broken session-id resolution but is out of Phase 8 scope (classification markers are also written by the mechanical revenium-classifier plugin hook); it was left at its original pre-Phase-8 state and is flagged as a separate pre-existing issue.
...
- **Regression test:** `test_job_marker_snippets_bind_explicit_session_id` added to `tests/test_repository.py` — extracts the two `execute_code` snippets containing `def write_job_marker` and asserts each carries the `REPLACE_WITH_YOUR_SESSION_ID` placeholder and guard and contains neither the `HERMES_SESSION_ID` env fallback nor the `os.listdir(sessions_dir)` glob. Scoped to job-marker snippets only.
- **Test count:** 48 → 49 (all passing).
- **Commits:** `fix(08-02): bind job markers to explicit agent-supplied session id` (initial fix, over-reached into the TASK CLASSIFICATION snippet), followed by a corrective commit reverting that snippet to its original state and rescoping the regression test to the job-marker snippets only.
