...
- [x] **Phase 4: Wire Enrichment** - Source `--operation-type` / `--agent` / `--trace-id` from marker fields; preserve provider inference for every split call. (Verified 2026-05-14 via UAT 5/5: wire ships `--operation-type CHAT` + `--agent Hermes` / `--trace-id <sid>` fallback per D-23, 8-provider regression pinned, D-25 attribution doc landed, Revenium dashboard shows varied content-driven task_type values once the classifier chain — D-07 removal, mint-first prompt, state.db content lookup — was deployed.) (2026-05-14)
- [x] **Phase 5: Housekeeping & Compat Hardening** - Marker file pruning, backward-compat regression tests, end-to-end test fixtures. Operational hygiene with no functional dependency.
- [x] **Phase 6: Mechanical Classification via Hermes agent:end Hook** - Replace soft prompt enforcement of FINAL ACTION with a Hermes lifecycle hook that classifies every turn and writes the marker file mechanically. Subagents inherit parent task_type via state.db `parent_session_id`. LLM-assisted classification using the budgeted model. Surfaced by Phase 3 UAT on the Mac Studio — agent-side adoption of the Phase 2 closing-discipline pattern is unreliable in Hermes' lazy-skill-loading + delegate_task subagent architecture. (Verified 2026-05-14 via UAT round 4: CLI substantive turn → marker with task_type=generation → sentinel → cron settle filter → Revenium received --task-type=generation per-marker. All 4 gaps closed: G-01 architectural, G-02 state.db tool-count, G-03 sentinel+settle, G-04 split conservation.) (completed 2026-05-14)
...
**Requirements**: HOOK-01, HOOK-02, HOOK-03, HOOK-04, HOOK-05, HOOK-06, HOOK-07, HOOK-08, HOOK-09, HOOK-10, HOOK-11, HOOK-12, HOOK-13
**Success Criteria** (what must be TRUE):
...
**Plans**: 4 plans (06-01 ships the original gateway-hook implementation, 06-02 closes G-01 with the hermes_cli plugin migration, 06-03 closes G-02 with the state.db tool-count source-of-truth swap, 06-04 closes G-03 with the plugin-sentinel + cron settle-window filter)
...
| 4. Wire Enrichment | 1/1 | Verified — UAT 5/5 passed (wire enrichment + classifier chain): `--operation-type CHAT` on every revenium call, `--agent`/`--trace-id` fallback per D-23, 8-provider regression pinned by `test_wire_no_provider_regression_per_class`, D-25 attribution paragraph in setup.md, Revenium dashboard shows content-driven task_type variety (`moltbook_heartbeat_check`, `hardware_overview`, etc.) after the D-07/mint-first/content-lookup quick tasks. | 2026-05-14 |
| 5. Housekeeping & Compat Hardening | 4/4 | Executed | 2026-05-14 |
...
- **Mapped to phases:** 50
- **Unmapped:** 0
- **Categories:** PATH (3), TAX (5), MARK (5), PROMPT (7), CRON (9), WIRE (4), COMPAT (4), TEST (5), HOOK (13)
