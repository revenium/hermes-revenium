...
| `test_runtime_paths_are_hermes_native` | `skills/revenium/scripts/common.sh` contains the literal strings `.hermes` and `state/revenium`, and does *not* contain the forked-tool runtime-path literal asserted absent on `tests/test_repository.py:55`. Locks the cron half into writing runtime state under `~/.hermes/state/revenium/`. | `tests/test_repository.py:51-55` |
